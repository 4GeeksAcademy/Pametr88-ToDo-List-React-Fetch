...
const Home = () => {
	...
	useEffect(() => { sendList() }, [todos]);
	const createUser = async () => {
		try {
			const response = await fetch('https://playground.4geeks.com/apis/fake/todos/user/', {
				method: "POST",
				body: JSON.stringify([]),
				headers: {
	...
	const pressEnter = async (e) => {
		try {
			if (e.key === "Enter" && inputValue !== "") {
				...
				setTodos([...todos, obj]);
				setInputValue(""); const final = await sendList()
	...
	return (<div className="Container">
		...
		<ul className="list-group list-group-flush">
			...
			{todos.map((item, index) => (<li className="list-group-item li-c" key={index}>
				{item.label}
				<button onClick={() => {
